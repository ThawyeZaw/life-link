...
export const LoginForm = () => {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      >
        <input
          type="email" placeholder={t("login.emailPlaceholder")} required value={email}
        ...
        />
        <input
          type="password" placeholder={t("login.passwordPlaceholder")} required value={password}
      ...
      <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
        <p className="mb-3 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500">
          <FlaskConical className="h-3.5 w-3.5" /> {t("login.demoAccounts")}
        </p>
        <div className="flex flex-col gap-2">
          {DEMO_ACCOUNTS.map((acc) => (
            ...
            >
              <span>{acc.label}</span>
              <span className="text-xs text-red-500">{t("login.tapLogin")}</span>
            </button>
          ))}
        </div>
      </div>
